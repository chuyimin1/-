...
public class TaskManager : MonoBehaviour
{
    ...
    public void ResetDailyState()
    {
        ...
        {
            // 确保手机系统的 UI 面板被重置
            phoneSystem.phoneSystemUI.SetActive(false);    // 隐藏对话界面

            // 强制重置按钮：这是解决“第二天依然是挂断按钮”的关键
            phoneSystem.nextButton.gameObject.SetActive(true);
            phoneSystem.hangUpButton.gameObject.SetActive(false);

            // 恢复按钮的交互性（防止卡在第一天末尾的禁用状态）
            phoneSystem.nextButton.interactable = true;
            if (phoneSystem.nextButtonImage != null)
            {
                phoneSystem.nextButtonImage.raycastTarget = true;
                phoneSystem.nextButtonImage.sprite = phoneSystem.lightSprite; // 变回亮色
            }

            // 调用你之前写的清理函数，确保旧消息彻底消失
            phoneSystem.LoadDialogueForDay(currentDay);
        }
    ...
    public void StartWork()
    {
        ...
        {
            // 1. 此时才开启手机物体
            phoneSystem.gameObject.SetActive(true);
            // 2. 此时才显示“来电”那个小面板
            phoneSystem.incomingCallPanel.SetActive(true);
            phoneSystem.phoneSystemUI.SetActive(false);
            // 3. 加载数据
            phoneSystem.LoadDialogueForDay(currentDay);
        }
